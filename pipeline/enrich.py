"""Threat-intelligence enrichment — KEV + EPSS + NVD + Exploit-DB.

All lookups are disk-cached. Fetcher is injectable for tests.
"""
...
class Fetcher:
    def get_json(self, url: str, headers: Optional[Dict] = None, timeout: int = 20) -> Any:
...
class Enricher:
    def __init__(self, cfg: Dict, cache_dir: Optional[str] = None, fetcher: Optional[Fetcher] = None):
        self.cfg = cfg
        self.cache_dir = cache_dir or cfg.get("cache_dir", "intel")
    ...
    def load_kev(self) -> None:
        cache = os.path.join(self.cache_dir, "cisa_kev.json")
        data = _load_cache(cache)
        if data and not _is_stale(cache, int(self.cfg.get("cache_ttl_days", 1))):
            self.kev_map = {v["cveID"]: v for v in data.get("vulnerabilities", [])}
            return
        try:
            data = _fetch_with_retry(self.fetcher.get_json, self.cfg["kev_url"])
    ...
    def load_epss(self, cves: List[str]) -> None:
        ...
        cache_path = os.path.join(self.cache_dir, "epss_cache.json")
        ...
        if missing:
            for chunk in _chunks(missing, 60):
                try:
                    ...
                    data = _fetch_with_retry(self.fetcher.get_json, f"{self.cfg['epss_url']}?{q}")
    ...
    def load_epss_trend(self, cves: List[str]) -> None:
        ...
        cache = _load_cache(cache_path) or {}
        if not isinstance(cache, dict) or cache.get("_queried_date") != past:
            cache = {"_queried_date": past}
        ...
        if missing:
            for chunk in _chunks(missing, 60):
                try:
                    ...
                    print(f"  ! EPSS trend chunk failed ({exc})")
            _save_cache(cache_path, cache)
        self.trend_map = cache

    def load_nvd(self, cves: List[str]) -> None:
        cache_path = os.path.join(self.cache_dir, "nvd_cache.json")
        cache = _load_cache(cache_path) or {}
        missing = [c for c in sorted({c.upper() for c in cves if c}) if c not in cache]
        if not missing:
            self.nvd_map = cache
        ...
        api_key = self.cfg.get("nvd_api_key") or os.environ.get("NVD_API_KEY") or ""
    ...
    def load_exploits_csv(self, cves: List[str]) -> None:
        ...
        if _is_stale(cache_path, max_age_days=1):
            try:
                raw = _fetch_with_retry(self.fetcher.get_raw, EXPLOITDB_CSV_URL, timeout=60)
        ...
        try:
            import re as _re
            cve_re = _re.compile(r"CVE-\d{4}-\d+", _re.IGNORECASE)
            with open(cache_path, encoding="utf-8", errors="replace") as fh:
                ...
                for row in reader:
                    ...
                    for token in aliases.replace(";", ",").split(","):
                        token = token.strip().upper()
                        if token.startswith("CVE-"):
                            cve_map.setdefault(token, []).append(eid)
        except Exception as exc:
    ...
    def enrich(self, findings: List[Finding], use_searchsploit: Optional[bool] = None) -> None:
        use_exploitdb = self.cfg.get("use_searchsploit", True) if use_searchsploit is None else use_searchsploit
        ...
        for f in findings:
            ...
            if f.kev and not f.exploit_available:
                f.exploit_available = True
