"""Run history persisted in SQLite — one row per product per pipeline run."""
...
class History:
    ...
    def history_for(self, product: str) -> List[Dict[str, Any]]:
        rows = self.conn.execute(
            ...
            "avg_score, top_score, p1, p2, p3, p4 FROM runs WHERE product=? ORDER BY run_date",
            (product,),
        ).fetchall()
        cols = ["run_date", "raw", "unique_findings", "quarantined", "final", "dedup_pct",
                "avg_score", "top_score", "p1", "p2", "p3", "p4"]
