...
(function buildHeader(){const S=DASH.summary;const meta=$('run-meta');const pills=[`<span class="meta-pill">🗓 ${S.run_date.substring(0,16)}</span>`,...S.products.map(p=>`<span class="meta-pill">📦 ${esc(p)}</span>`)];if(S.p1>0)pills.push(`<span class="meta-pill warning">🚨 ${S.p1} P1</span>`);meta.innerHTML=pills.join('');$('tc-findings').textContent=S.final_findings;$('tc-quarantine').textContent=S.quarantined;})();
(function buildBrief(){const brief=DASH.executive_brief;if(!brief){$('brief-section').style.display='none';return;}$('brief-section').innerHTML=`<div class="brief-card"><div class="brief-icon">🤖</div><div><div class="brief-label">AI Executive Brief</div><div class="brief-text">${esc(brief)}</div></div></div>`;})();
(function buildKPIs(){const S=DASH.summary;const noiseRm=S.raw_findings>0?((S.raw_findings-S.final_findings)/S.raw_findings*100).toFixed(1):0;const cards=[{v:S.raw_findings,l:'Raw findings',sub:'before any processing',cls:'blue',icon:'📥'},{v:S.unique_findings,l:'After dedup',sub:`${S.dedup_pct}% duplicates removed`,cls:'cyan',icon:'🔀'},{v:S.quarantined,l:'Quarantined',sub:'FP / accepted risk',cls:'',icon:'🚫'},{v:S.final_findings,l:'Active findings',sub:'prioritised & scored',cls:'warn',icon:'🎯'},{v:S.p1+S.p2,l:'P1 + P2 tickets',sub:`${S.p1} critical · ${S.p2} high`,cls:'danger',icon:'🚨'},{v:S.avg_score,l:'Avg risk score',sub:`top score: ${S.top_score}`,cls:'',icon:'📊'},{v:noiseRm+'%',l:'Noise removed',sub:'raw → final reduction',cls:'success',icon:'📉'}];const grid=$('kpi-grid');grid.innerHTML=cards.map((c,i)=>`<div class="kpi ${c.cls}"><span class="kpi-accent">${c.icon}</span><div class="kpi-value" id="kv-${i}">${typeof c.v==='number'?'0':c.v}</div><div class="kpi-label">${c.l}</div><div class="kpi-sub">${c.sub}</div></div>`).join('');cards.forEach((c,i)=>{if(typeof c.v==='number')animateCount($(`kv-${i}`),c.v);});})();
(function initCharts(){const S=DASH.summary,F=DASH.findings,H=DASH.history;
...
function scanProduct(id){$('ap-msg').textContent='Scan triggered for '+id+' (re-run pipeline to scan)';$('ap-msg').style.color='#3b82f6';}
...
(function initLifecycle(){
  ...
  $('tc-lifecycle').textContent=lcFindings.length;
  // Lifecycle table
  ...
  $('lc-table-wrap').innerHTML=`<table class="f-table"><thead><tr><th>Product</th><th>Severity</th><th>Title</th><th>CVE</th><th>Status</th><th>SLA</th><th>Owner</th></tr></thead><tbody>${rows}</tbody></table>`;
  // Breached list
  const breached=lcFindings.filter(f=>f.breached);
  $('lc-breached-list').innerHTML=breached.length?breached.map(f=>`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.03);font-size:12px"><span style="color:#ef4444;font-weight:700">⏰</span><span style="flex:1"><b>${esc(f.title.substring(0,50))}</b> <span class="dimmed">${esc(f.product)}</span></span><span class="badge b-p1">SLA BREACHED</span></div>`).join(''):'<p class="dimmed" style="font-size:12px">No SLA breaches detected.</p>';
})();
...
(function initDedup(){
  ...
  $('dedup-overlap-table').innerHTML=overlapRows?`<table class="f-table"><thead><tr><th>CVE</th><th>Detecting Scanners</th><th>Vulnerability</th><th>Product</th></tr></thead><tbody>${overlapRows}</tbody></table>`:'<p class="dimmed" style="font-size:12px">No cross-scanner overlaps detected.</p>';
})();
