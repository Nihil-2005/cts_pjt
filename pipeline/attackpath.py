"""Attack-path model based on CAPEC CWE→CWE transition graph."""
...
CHAINS: List[tuple] = [
    ("CWE-200", "CWE-287", 0.45, "Exposed sensitive data reveals credentials enabling auth bypass.", "CAPEC-115 Authentication Bypass"),
    ...
    ("CWE-287", "CWE-284", 0.70, "Auth bypass grants unauthorized access.", "CAPEC-115 Authentication Bypass"),
    ("CWE-284", "CWE-94", 0.30, "Unauthorized privileged access exposes injection surface.", "CAPEC-233 Privilege Escalation"),
    ("CWE-287", "CWE-94", 0.30, "Auth bypass reaches injection-capable functionality.", "CAPEC-233 Privilege Escalation"),
    ("CWE-287", "CWE-434", 0.40, "Auth bypass reaches administrative file upload.", "CAPEC-650 Upload a Web Shell"),
    ("CWE-89", "CWE-94", 0.50, "SQL injection enables code execution.", "CAPEC-66 SQL Injection"),
    ...
    ("CWE-79", "CWE-352", 0.55, "XSS drives CSRF state-changing requests.", "CAPEC-86 XSS"),
    ("CWE-352", "CWE-284", 0.50, "CSRF performs unauthorized state-changing actions.", "CAPEC-62 CSRF"),
    ("CWE-352", "CWE-434", 0.30, "CSRF triggers file upload on admin functions.", "CAPEC-62 CSRF"),
    ...
    ("CWE-918", "CWE-284", 0.60, "SSRF reaches internal services bypassing network ACLs.", "CAPEC-664 SSRF"),
    ("CWE-918", "CWE-200", 0.65, "SSRF reads cloud metadata / internal sensitive data.", "CAPEC-664 SSRF"),
    ("CWE-918", "CWE-94", 0.40, "SSRF to metadata yields credentials enabling code execution.", "CAPEC-664 SSRF"),
    ...
    ("CWE-611", "CWE-200", 0.70, "XXE reads local files / internal resources.", "CAPEC-652 XXE"),
    ("CWE-611", "CWE-918", 0.50, "XXE leveraged for SSRF.", "CAPEC-652 XXE"),
...
IMPACT_CWES = {"CWE-94", "CWE-78", "CWE-434", "CWE-502", "CWE-89", "CWE-918", "CWE-287", "CWE-284", "CWE-522", "CWE-200"}
...
def build_attack_paths(findings: List[Finding], product: str, product_cfg: Dict) -> List[AttackPath]:
    ...
    for from_cwe, to_cwe, chainability, desc, ref in CHAINS:
        ...
        paths.append(AttackPath(
            ...
            chainability=chainability, probability=prob, factors=factors,
            capec_ref=ref, description=desc,
        ))
    return paths


def attach_escalation_potential(findings: List[Finding], paths: List[AttackPath], product: Optional[str] = None) -> None:
    ...
    for f in findings:
        if product is not None and f.product != product:
            continue
        if f.status != "active":
            continue
        if f.cwe:
